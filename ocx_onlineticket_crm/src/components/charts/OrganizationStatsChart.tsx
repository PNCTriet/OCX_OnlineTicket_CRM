...
interface OrganizationStatsChartProps {
...
export default function OrganizationStatsChart({ 
  ...
  const fetchOrganizationStats = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/dashboard/organization/${organizationId}`, {
        headers: {
  ...
  if (loading || chartLoading) {
    return (
      <Card className="dark:bg-white/[0.03] dark:border-gray-800">
        <Title className="dark:text-white/90">{organizationName} Stats</Title>
  ...
  const progressData = [
    {
      ...
      percentage: Math.min((stats.total_revenue / 10000000) * 100, 100),
    },
    {
      name: "Tickets Sold",
      value: stats.total_tickets_sold,
      target: 1000,
      percentage: Math.min((stats.total_tickets_sold / 1000) * 100, 100),
    },
    {
      name: "Orders",
      value: stats.total_orders,
      target: 500,
      percentage: Math.min((stats.total_orders / 500) * 100, 100),
    },
    {
      name: "Events",
      value: stats.total_events,
      target: 50,
      percentage: Math.min((stats.total_events / 50) * 100, 100),
    },
  ];
    ...
    <Card className="dark:bg-white/[0.03] dark:border-gray-800">
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        ...
        <Card className="dark:bg-white/[0.02] dark:border-gray-700">
          ...
          <Text className="dark:text-gray-400">Total Events</Text>
        </Card>
      </div>
    </Card>
  );
