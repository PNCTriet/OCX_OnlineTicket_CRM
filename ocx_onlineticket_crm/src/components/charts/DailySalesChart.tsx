...
export default function DailySalesChart({ data, loading = false }: DailySalesChartProps) {
  ...
  const getMaxRevenue = (data: DailySalesData[]) => {
    if (data.length === 0) return 0;
    return Math.max(...data.map(d => d.revenue));
  };

  const getYAxisLabels = (data: DailySalesData[]) => {
                    ...
                    return (
                      <div key={index} className="flex flex-col items-center h-full">
                        <div className="flex-1 flex items-end justify-center">
                          <div 
                            className="w-4 bg-green-500 rounded-t-sm transition-all duration-300 hover:bg-green-600 relative group"
                            style={{ 
                              height: `${height}%`, 
                              minHeight: '4px',
                              alignSelf: 'flex-end'
